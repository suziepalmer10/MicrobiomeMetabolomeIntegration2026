# Cholesterol ML pipeline analysis
# ------------------------------------------------------------
# Purpose:
#   Read all seed-level CSV files for the Erawijantari Cholesterol analysis,
...
#
# Notes:
#   - Test SD/CI inside a single file may be NA because each seed has one test set,
#     but across-seed mean/CI are still computed here.
#   - Heatmaps print values to 4 decimal places.
#   - Output filenames are prefixed with the outcome label, e.g.:
#       Erawijantari_Cholesterol_performance_test_rmse.png
#       Erawijantari_Cholesterol_heatmap_test_rmse.png
#       Erawijantari_Cholesterol_winner_category_frequency_test_rmse.png
#
# ------------------------------------------------------------
...
main_metrics <- c("R2", "RMSE", "MAE", "MAPE", "MEDIAN_ABS_ERROR")
metric_direction <- c(
  R2 = "higher",
  RMSE = "lower",
  MAE = "lower",
  MAPE = "lower",
  MEDIAN_ABS_ERROR = "lower"
)
...
parse_filename <- function(path) {
  ...
  m <- stringr::str_match(
    nm,
    "^([^_]+_[^_]+)__model-([^_]+)__metabT-([^_]+)__taxaT-([^_]+)__metabR-([^_]+)__taxaR-([^_]+)_run(\\d{2})\\.csv$"
  )
...
read_one_file <- function(path) {
  ...
  dat %>%
    ...
    pivot_longer(
      cols = -category,
      names_to = c("metric", "split", "stat"),
      names_pattern = "^(.*)_(train|test)_(mean|sd|n|ci_lower|ci_upper)$",
      values_to = "value"
    ) %>%
...
pick_primary_rows <- function(long_df, mode = "best_per_seed", fixed_category = "concat_none") {
  test_r2 <- long_df %>%
    filter(split == "Test", metric == "R2") %>%
...
message("Matched cholesterol seed-result CSVs: ", length(files))
...
plot_long <- pick_primary_rows(
  ...
  mutate(
    metric = case_when(
      metric == "MEDIAN_ABS_ERROR" ~ "MedianAE",
      TRUE ~ metric
    )
  )

plot_metrics <- c("RMSE", "MAE", "MedianAE", "R2")

# -----------------------------
# Across-seed summaries for the 8 dot-whisker plots
# -----------------------------
plot_summary <- plot_long %>%
  ...
  mutate(
    direction = case_when(
      metric %in% c("RMSE", "MAE", "MedianAE") ~ "lower",
      metric == "R2" ~ "higher",
      TRUE ~ NA_character_
    )
  )
...
make_metric_plot <- function(df, metric_name, split_name) {
  ...
  d <- df %>%
    ...
    filter(!is.na(mean)) %>%
    mutate(pipeline_id = as.character(pipeline_id))
  
  if (nrow(d) == 0) {
    return(
      ggplot() +
        annotate("text", x = 1, y = 1, label = paste("No data for", split_name, metric_name), size = 5) +
        theme_void() +
        labs(title = paste(split_name, metric_name))
    )
  }
  
  if (metric_name %in% c("RMSE", "MAE", "MedianAE")) {
    ord <- d %>%
      arrange(mean) %>%
      pull(pipeline_id) %>%
      unique()
    ...
    ord <- unique(d$pipeline_id)
  }
  
  d <- d %>%
    mutate(pipeline_id = factor(pipeline_id, levels = rev(ord)))
  
  y_lab <- case_when(
    metric_name == "RMSE" ~ "RMSE (mean across seeds, 95% CI)",
    metric_name == "MAE" ~ "MAE (mean across seeds, 95% CI)",
    metric_name == "MedianAE" ~ "MedianAE (mean across seeds, 95% CI)",
    metric_name == "R2" ~ "R² (mean across seeds, 95% CI)",
    TRUE ~ "Mean across seeds (95% CI)"
  )
  ...
  ggplot(d, aes(x = pipeline_id, y = mean)) +
    ...
    coord_flip() +
    labs(
      title = paste(split_name, metric_name),
...
train_panel <- (
  plot_list$plot_obj[[which(plot_list$split == "Train" & plot_list$metric == "RMSE")]] +
    plot_list$plot_obj[[which(plot_list$split == "Train" & plot_list$metric == "MAE")]]
) / (
  plot_list$plot_obj[[which(plot_list$split == "Train" & plot_list$metric == "MedianAE")]] +
    plot_list$plot_obj[[which(plot_list$split == "Train" & plot_list$metric == "R2")]]
)

test_panel <- (
  plot_list$plot_obj[[which(plot_list$split == "Test" & plot_list$metric == "RMSE")]] +
    plot_list$plot_obj[[which(plot_list$split == "Test" & plot_list$metric == "MAE")]]
) / (
  plot_list$plot_obj[[which(plot_list$split == "Test" & plot_list$metric == "MedianAE")]] +
    plot_list$plot_obj[[which(plot_list$split == "Test" & plot_list$metric == "R2")]]
)
...
# -----------------------------
heatmap_metrics <- c("RMSE", "MAE", "MEDIAN_ABS_ERROR", "R2")
...
make_perf_heatmap <- function(df, split_name, metric_name) {
  ...
  if (metric_name %in% c("RMSE", "MAE", "MEDIAN_ABS_ERROR")) {
    d <- d %>% mutate(pipeline_id = fct_reorder(pipeline_id, mean_across_seeds, .desc = TRUE))
  } else {
    d <- d %>% mutate(pipeline_id = fct_reorder(pipeline_id, mean_across_seeds, .desc = FALSE))
  }
...
# -----------------------------
# Winner category frequency for RMSE, MAE, Med-AE
# -----------------------------
winner_metrics <- c("RMSE", "MAE", "MEDIAN_ABS_ERROR")
...
winner_plot_grid <- tidyr::crossing(
  split = c("Train", "Test"),
  metric_pretty = c("RMSE", "MAE", "Med-AE")
) %>%
  mutate(
...
train_winner_panel <- (
  winner_plot_grid$plot_obj[[which(winner_plot_grid$split == "Train" & winner_plot_grid$metric_pretty == "RMSE")]] /
    winner_plot_grid$plot_obj[[which(winner_plot_grid$split == "Train" & winner_plot_grid$metric_pretty == "MAE")]] /
    winner_plot_grid$plot_obj[[which(winner_plot_grid$split == "Train" & winner_plot_grid$metric_pretty == "Med-AE")]]
)

test_winner_panel <- (
  winner_plot_grid$plot_obj[[which(winner_plot_grid$split == "Test" & winner_plot_grid$metric_pretty == "RMSE")]] /
    winner_plot_grid$plot_obj[[which(winner_plot_grid$split == "Test" & winner_plot_grid$metric_pretty == "MAE")]] /
    winner_plot_grid$plot_obj[[which(winner_plot_grid$split == "Test" & winner_plot_grid$metric_pretty == "Med-AE")]]
)
...
html_lines <- c(
  ...
  "<div class='section'>",
  "<h2>Heatmaps</h2>",
  paste0("<h3>Train RMSE</h3><img src='plots/", outcome_label, "_heatmap_train_rmse.png'/>"),
  paste0("<h3>Train MAE</h3><img src='plots/", outcome_label, "_heatmap_train_mae.png'/>"),
  paste0("<h3>Train Med-AE</h3><img src='plots/", outcome_label, "_heatmap_train_median_abs_error.png'/>"),
  paste0("<h3>Train R2</h3><img src='plots/", outcome_label, "_heatmap_train_r2.png'/>"),
  paste0("<h3>Test RMSE</h3><img src='plots/", outcome_label, "_heatmap_test_rmse.png'/>"),
  paste0("<h3>Test MAE</h3><img src='plots/", outcome_label, "_heatmap_test_mae.png'/>"),
  paste0("<h3>Test Med-AE</h3><img src='plots/", outcome_label, "_heatmap_test_median_abs_error.png'/>"),
  paste0("<h3>Test R2</h3><img src='plots/", outcome_label, "_heatmap_test_r2.png'/>"),
  "</div>",
