...
base_dir <- "/Users/suzettepalmer/Library/CloudStorage/Box-Box/Personal - Suzette Palmer/IntegratedLearner_Revisions/Results/Performance_CSVs/Binary"
...
for (dataset in datasets) {
  ...
  summary_df <- df %>%
    filter(
      metric %in% c("ACCURACY", "AUROC", "AUPRC", "SENSITIVITY", "SPECIFICITY",
  ...
  for (split_value in c("Train", "Test")) {
    for (metric_value in c("ACCURACY", "AUROC", "AUPRC", "SENSITIVITY", "SPECIFICITY",
                           "PRECISION", "F1", "BALANCED_ACCURACY")) {
