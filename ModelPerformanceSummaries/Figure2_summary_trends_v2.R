...
#
#   best_value = min(mean_across_seeds)
#   keep if mean_across_seeds <= best_value * 1.02
#
# Counting modes:
#   raw:
#     each qualifying configuration counts 1,
#     BUT exact ties at the same performance value share credit 1/n_ties
#
#   dataset_equal:
#     each dataset contributes total credit 1 within each metric,
#     split evenly across all qualifying configurations
# ============================================================
...
setwd('/Users/suzettepalmer/Library/CloudStorage/Box-Box/Personal - Suzette Palmer/IntegratedLearner_Revisions/Results/Performance_CSVs/')
file_map <- c(
  "Erawijantari_Cholesterol" = "/Users/suzettepalmer/Library/CloudStorage/Box-Box/Personal - Suzette Palmer/IntegratedLearner_Revisions/Results/Performance_CSVs/Erawijantari_Cholesterol/results/tables/Erawijantari_Cholesterol_heatmap_summary.csv",
  "Erawijantari_Glucose"     = "/Users/suzettepalmer/Library/CloudStorage/Box-Box/Personal - Suzette Palmer/IntegratedLearner_Revisions/Results/Performance_CSVs/Erawijantari_Glucose/results/tables/Erawijantari_Glucose_heatmap_summary.csv",
  "Franzosa_CD_Fp"           = "/Users/suzettepalmer/Library/CloudStorage/Box-Box/Personal - Suzette Palmer/IntegratedLearner_Revisions/Results/Performance_CSVs/Franzosa_CD_Fp/results/tables/Franzosa_CD_Fp_heatmap_summary.csv", 
  "Franzosa_IBD_Fp"           = "/Users/suzettepalmer/Library/CloudStorage/Box-Box/Personal - Suzette Palmer/IntegratedLearner_Revisions/Results/Performance_CSVs/Franzosa_IBD_Fp/results/tables/Franzosa_IBD_Fp_heatmap_summary.csv",
  "Franzosa_UC_Fp"           = "/Users/suzettepalmer/Library/CloudStorage/Box-Box/Personal - Suzette Palmer/IntegratedLearner_Revisions/Results/Performance_CSVs/Franzosa_UC_Fp/results/tables/Franzosa_UC_Fp_heatmap_summary.csv",
  "Wang_Creatinine"           = "/Users/suzettepalmer/Library/CloudStorage/Box-Box/Personal - Suzette Palmer/IntegratedLearner_Revisions/Results/Performance_CSVs/Wang_creatinine/results/tables/Wang_creatinine_heatmap_summary.csv",
  "Wang_eGFR"           = "/Users/suzettepalmer/Library/CloudStorage/Box-Box/Personal - Suzette Palmer/IntegratedLearner_Revisions/Results/Performance_CSVs/Wang_eGFR/results/tables/Wang_eGFR_heatmap_summary.csv",
  "Wang_Urea"           = "/Users/suzettepalmer/Library/CloudStorage/Box-Box/Personal - Suzette Palmer/IntegratedLearner_Revisions/Results/Performance_CSVs/Wang_Urea/results/tables/Wang_urea_heatmap_summary.csv"
)

dataset_pretty <- c(
  "Erawijantari_Cholesterol" = "Cholesterol",
  "Erawijantari_Glucose"     = "Glucose",
  "Franzosa_CD_Fp"           = "CD_Fp",
  "Franzosa_IBD_Fp"           = "IBD_Fp",
  "Franzosa_UC_Fp"           = "UC_Fp",
  "Wang_Creatinine"         = "Creatinine",
  "Wang_eGFR"         = "eGFR", 
  "Wang_Urea" = "Urea"
...
# ------------------------------------------------------------
# 4) Keep test RMSE and MAE only
# ------------------------------------------------------------
test_dat <- all_dat %>%
  filter(split == "Test", metric %in% c("RMSE", "MAE")) %>%
  mutate(
...
competitive_set <- test_dat %>%
  ...
  mutate(
    best_value = min(mean_across_seeds, na.rm = TRUE),
    threshold_value = best_value * (1 + threshold_pct),
    delta_from_best = mean_across_seeds - best_value,
    pct_from_best = (mean_across_seeds - best_value) / best_value
  ) %>%
...
# 10) Build plots
# ------------------------------------------------------------
p_rmse_bar_raw <- make_bar_panel(competitive_set, "RMSE", mode = "raw")
p_mae_bar_raw  <- make_bar_panel(competitive_set, "MAE", mode = "raw")

p_rmse_bar_equal <- make_bar_panel(competitive_set, "RMSE", mode = "dataset_equal")
p_mae_bar_equal  <- make_bar_panel(competitive_set, "MAE", mode = "dataset_equal")

p_rmse_tile <- make_competitive_tile_plot(competitive_set, "RMSE")
p_mae_tile  <- make_competitive_tile_plot(competitive_set, "MAE")

print(p_rmse_bar_raw)
print(p_mae_bar_raw)
print(p_rmse_bar_equal)
print(p_mae_bar_equal)
print(p_rmse_tile)
print(p_mae_tile)
...
# 11) Save plots
# ------------------------------------------------------------
#ggsave("RMSE_bar_summary_within_2pct_raw.png", p_rmse_bar_raw, width = 15, height = 12, dpi = 300)
ggsave("RMSE_bar_summary_within_2pct_raw.pdf", p_rmse_bar_raw, width = 15, height = 12)

#ggsave("MAE_bar_summary_within_2pct_raw.png", p_mae_bar_raw, width = 15, height = 12, dpi = 300)
ggsave("MAE_bar_summary_within_2pct_raw.pdf", p_mae_bar_raw, width = 15, height = 12)

#ggsave("RMSE_bar_summary_within_2pct_dataset_equal.png", p_rmse_bar_equal, width = 15, height = 12, dpi = 300)
ggsave("RMSE_bar_summary_within_2pct_dataset_equal.pdf", p_rmse_bar_equal, width = 15, height = 12)

#ggsave("MAE_bar_summary_within_2pct_dataset_equal.png", p_mae_bar_equal, width = 15, height = 12, dpi = 300)
ggsave("MAE_bar_summary_within_2pct_dataset_equal.pdf", p_mae_bar_equal, width = 15, height = 12)

#ggsave("RMSE_competitive_set_tile_plot.png", p_rmse_tile, width = 14, height = 9, dpi = 300)
ggsave("RMSE_competitive_set_tile_plot.pdf", p_rmse_tile, width = 14, height = 9)

#ggsave("MAE_competitive_set_tile_plot.png", p_mae_tile, width = 14, height = 9, dpi = 300)
ggsave("MAE_competitive_set_tile_plot.pdf", p_mae_tile, width = 14, height = 9)
...
freq_table <- bind_rows(
  make_frequency_table(competitive_set, "RMSE", mode = "raw"),
  make_frequency_table(competitive_set, "MAE", mode = "raw"),
  make_frequency_table(competitive_set, "RMSE", mode = "dataset_equal"),
  make_frequency_table(competitive_set, "MAE", mode = "dataset_equal")
)
...
# ------------------------------------------------------------
# 13) Save heatmap-style Excel tables
# ------------------------------------------------------------
library(openxlsx)
library(dplyr)
library(tidyr)
...
# This is the value shown inside each heatmap cell
heatmap_value_col <- "mean_across_seeds"

# Match the column order shown in the PDF heatmaps
category_levels <- c(
  "concat_none",
  "concat_scaling",
...
for (ds in names(dataset_pretty)) {
  ...
  for (metric_name in c("RMSE", "MAE")) {
    for (split_name in c("Train", "Test")) {
      ...
      sheet_name <- substr(paste0(ds_label, "_", metric_name, "_", split_name), 1, 31)
