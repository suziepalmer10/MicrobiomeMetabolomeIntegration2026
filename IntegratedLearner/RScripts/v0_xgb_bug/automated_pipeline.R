...
run_one <- function(args, run_id, seed, return_big = FALSE) {
  ...
  extract_fold_importance <- function(fit_obj, model_type, view_name,
    ...
    if (model_type == "xgboost") {
      ...
      # caret stores predictors in trainingData; first column is outcome
      #feat_names <- colnames(fit$trainingData)[-1]
      feat_names <- fit$feature_names
      
      xi <- tryCatch(
        xgboost::xgb.importance(feature_names = feat_names, model = booster),
