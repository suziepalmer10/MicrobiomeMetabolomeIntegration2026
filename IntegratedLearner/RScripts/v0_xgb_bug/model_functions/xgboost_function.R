fit <- function(trainData, response_variable, response_type,
  ...
  if (anyNA(x)) {
    ...
    stop("Predictor matrix contains NA values. Columns with NA: ",
         paste(bad_cols, collapse = ", "))
  }
  
  non_numeric <- names(x)[!vapply(x, is.numeric, logical(1))]
  if (length(non_numeric) > 0) {
    stop("All predictors must be numeric for XGBoost. Non-numeric columns: ",
  ...
  if (response_type == "binary") {
    ...
    if (anyNA(y_factor)) stop("Binary response contains NA values.")
    ...
    if (cv_folds_used < cv_folds) {
      warning("Reduced cv_folds from ", cv_folds, " to ", cv_folds_used,
              " due to small class count in this training fold.")
    }
    
    inner_index_train <- caret::createFolds(y_factor, k = cv_folds_used, returnTrain = TRUE)
    all_idx <- seq_len(length(y_num))
    xgb_folds <- lapply(inner_index_train, function(tr_idx) setdiff(all_idx, tr_idx))
    ...
    if (anyNA(y_num)) stop("Response contains NA values (continuous).")
  ...
  if (isTRUE(use_gpu)) {
    base_params$device <- gpu_device   # "cuda" or "cuda:0"
  } else {
  ...
  for (i in seq_len(nrow(tune_grid))) {
    ...
    if (!metric_col %in% colnames(elog)) {
      stop("Expected CV metric column '", metric_col, "' not found in evaluation_log. ",
...
predict <- function(fit, newData, response_variable, response_type,
  ...
  if (anyNA(x_new)) {
    ...
    stop("Prediction matrix contains NA values. Columns with NA: ",
         paste(bad_cols, collapse = ", "))
  }
  
  non_numeric <- names(x_new)[!vapply(x_new, is.numeric, logical(1))]
  if (length(non_numeric) > 0) {
    stop("All predictors in newData must be numeric for XGBoost. Non-numeric columns: ",
         paste(non_numeric, collapse = ", "))
  }
  ...
  if (length(missing_features) > 0) {
    stop("newData is missing required predictor columns: ",
  ...
  p <- stats::predict(fit$finalModel, newdata = dnew)
  
  if (response_type == "binary") {
    y_true <- as.factor(y_true)
    if (anyNA(y_true)) stop("Binary truth contains NA values.")
    ...
    return(list(pred = as.numeric(p), truth = as.numeric(truth)))
  }
  
  if (response_type == "continuous") {
    truth <- as.numeric(y_true)
    if (anyNA(truth)) stop("Continuous truth contains NA values.")
