...
binary_subdirs <- c(
  ...
  "Franzosa_con_vs_IBD",
  "Franzosa_con_vs_UC",
  "Wang_convskidneyfailure",
  "Yachida_control_vs_CRC",
  "Yachida_control_vs_MP_stage0",
  "Yachida_control_vs_stage1_2",
  "Yachida_control_vs_stage3_4"
)
...
binary_output_dir <- file.path(
  summary_root_dir,
  paste0("_binary_", top_n_label, "_aggregate_24x24_condition_heatmaps")
)

dir.create(binary_output_dir, recursive = TRUE, showWarnings = FALSE)

# Main choice:
# 1.0 means a heatmap cell is colored only if all loaded binary datasets have a valid value.
# 0.8 means at least 80% of loaded binary datasets must have a valid value.
# 0.5 means at least half of loaded binary datasets must have a valid value.
...
# Locate and load binary pairwise overlap files

binary_dataset_map <- tibble(
  RequestedSubdir = binary_subdirs,
  DatasetDir = file.path(summary_root_dir, binary_subdirs),
  Found = dir.exists(file.path(summary_root_dir, binary_subdirs))
)

write_csv(
  binary_dataset_map,
  file.path(binary_output_dir, paste0("binary_", top_n_label, "_dataset_input_resolution.csv")),
  na = ""
)

missing_dirs <- binary_dataset_map %>%
  filter(!Found)

if (nrow(missing_dirs) > 0) {
  warning(
    "The following binary dataset directories were not found:\n",
...
for (i in seq_len(nrow(binary_dataset_map))) {
  this_subdir <- binary_dataset_map$RequestedSubdir[i]
  this_dir <- binary_dataset_map$DatasetDir[i]
  this_found <- binary_dataset_map$Found[i]
...
write_csv(
  load_status_tbl,
  file.path(binary_output_dir, paste0("binary_", top_n_label, "_load_status.csv")),
  na = ""
)

if (length(loaded_pairwise) == 0) {
  stop("No binary pairwise overlap files were loaded. Check the load-status CSV.")
}

all_pairwise <- bind_rows(loaded_pairwise)

n_loaded_binary_datasets <- all_pairwise %>%
  distinct(RequestedSubdir) %>%
  nrow()

min_valid_datasets_per_cell <- ceiling(
  min_valid_dataset_fraction * n_loaded_binary_datasets
)

message("Loaded binary datasets: ", n_loaded_binary_datasets)
message("Minimum valid datasets per colored summary cell: ", min_valid_datasets_per_cell)

write_csv(
  all_pairwise,
  file.path(binary_output_dir, paste0("binary_", top_n_label, "_all_pairwise_overlap_loaded.csv")),
  na = ""
)
...
write_csv(
  condition_info,
  file.path(binary_output_dir, paste0("binary_", top_n_label, "_condition_info.csv")),
  na = ""
)
...
write_csv(
  ...
  file.path(binary_output_dir, paste0("binary_", top_n_label, "_condition_counts_by_view.csv")),
  na = ""
)
...
write_csv(
  ...
  file.path(binary_output_dir, paste0("binary_", top_n_label, "_dataset_condition_pair_values.csv")),
  na = ""
)


# Aggregate condition-pair values across binary datasets
...
aggregate_condition_pair_summary <- condition_grid %>%
  ...
  summarise(
    n_possible_binary_datasets = n_loaded_binary_datasets,
    
    n_valid_binary_datasets =
  ...
  mutate(
    ...
    valid_summary_cell =
      is_self_condition |
      n_valid_binary_datasets >= min_valid_datasets_per_cell,
...
write_csv(
  ...
  file.path(
    binary_output_dir,
    paste0("binary_", top_n_label, "_aggregate_condition_pair_summary_across_datasets.csv")
  ),
  na = ""
)
...
write_csv(
  ...
  file.path(
    binary_output_dir,
    paste0("binary_", top_n_label, "_invalid_or_self_summary_cells.csv")
  ),
  na = ""
)
  ...
  p <- ggplot(
    ...
    labs(
      ...
      caption = paste0(
        ...
        " valid binary datasets for that condition pair. ",
...
median_multipage_file <- file.path(
  binary_output_dir,
  paste0("figure_binary_", top_n_label, "_MEDIAN_aggregate_24x24_all_views_multipage.pdf")
)
...
mean_multipage_file <- file.path(
  binary_output_dir,
  paste0("figure_binary_", top_n_label, "_MEAN_aggregate_24x24_all_views_multipage.pdf")
)
...
summary_cell_diagnostics <- aggregate_condition_pair_summary %>%
  ...
  summarise(
    ...
    median_n_valid_binary_datasets =
      safe_median(n_valid_binary_datasets[!is_self_condition]),
    
    min_n_valid_binary_datasets =
      min(n_valid_binary_datasets[!is_self_condition], na.rm = TRUE),
    
    max_n_valid_binary_datasets =
      max(n_valid_binary_datasets[!is_self_condition], na.rm = TRUE),
...
write_csv(
  ...
  file.path(
    binary_output_dir,
    paste0("binary_", top_n_label, "_summary_cell_diagnostics_by_view.csv")
  ),
  na = ""
)
...
message("\nDone.")
message("Binary aggregate 24 x 24 heatmap output directory: ", binary_output_dir)
message("Main files created:")
message("  - figure_binary_", top_n_label, "_MEDIAN_aggregate_24x24_all_views_multipage.pdf")
message("  - figure_binary_", top_n_label, "_MEAN_aggregate_24x24_all_views_multipage.pdf")
message("  - Individual median and mean PDF heatmaps for each View")
message("  - binary_", top_n_label, "_aggregate_condition_pair_summary_across_datasets.csv")
message("  - binary_", top_n_label, "_dataset_condition_pair_values.csv")
message("  - binary_", top_n_label, "_summary_cell_diagnostics_by_view.csv")